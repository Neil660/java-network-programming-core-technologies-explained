...
public class StoreGui {
    ...
    public void refreshCustPane(Customer cust) {
        ...
        idTf.setText(Long.valueOf(cust.getId()).toString());
        nameTf.setText(cust.getName().trim());
        addrTf.setText(cust.getAddr().trim());
        ageTf.setText(Integer.valueOf(cust.getAge()).toString());
    }
    ...
    public void refreshAllCustPan(Set<Customer> custs) {
        ...
        while (it.hasNext()) {
            ...
            newData[i][0] = Long.valueOf(cust.getId()).toString();
            newData[i][1] = cust.getName();
            newData[i][2] = cust.getAddr();
            newData[i][3] = Integer.valueOf(cust.getAge()).toString();
            i++;
        }
